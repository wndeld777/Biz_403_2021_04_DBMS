...
-- 1년동안 매입과 매출금액을 계산하고 단순이익금 계산해보기
SELECT
    SUM(DECODE(io_inout,'매입',io_total)) AS 매입합계,
    SUM(DECODE(io_inout,'매출',io_total)) AS 매출합계,
...
-- 실제 20KG 의 쌀을 직접 판매하는 것보다 더 비싼 가격에 판매하게 된다
...
--        매출금액은 VAT 포함된 금액이다
-- 매입과 매출 데이터로 지난 1년간 납부한 VAT금액을 계산해 보자
-- 매입금액 : 22737397
-- 매출금액 : 41683800
...
-- 거래처별로 매입과 매출 합계
-- DECODE(칼럼명, 조건값, true 일때, false일때)
-- 실제 저장된 데이터를 PIVOT 으로 보여주기
SELECT io_dname, SUM(DECODE(io_inout,'매입',io_total,0)) AS 매입합계,
