...
VALUES('12345','홍길동',1231,'010-1234-5678','광주광역시 북구');
...
-- tbl_book_rent와 tbl_book, tbl_buyer table을 참조무결성 설정
-- 대상 Table 은 다중관계의 table
...
REFERENCES tbl_book(bk_isbn); -- 1 table과 칼럼
...
오라클 이외의 다른 DBMS에는 일련번호를 자동으로 만들어주는 Table 속성이 있다
오라클(11 이하)에서는 그러한 속성이 없다

mySQL의 경우 칼럼 속성에 AUTO_INCREMENT 로 설정을 하면
그 칼럼은 INSERT할때 별도 값을 지정하지 않아도
항상 유일한 일련번호로 자동생성이 된다
...
INSERT INTO tbl_book_rent(br_seq,br_sdate,br_isbn,br_bcode)
VALUES(seq_book_rent.NEXTVAL,'2021-04-20','9791162540756','B0020');

SELECT
    *
FROM tbl_book_rent;

-- tbl_book_rent 는 회원이 도서를 대여한 리스트가 있다
-- 여기에는 도서코드, 회원코드만 있기 때문에
-- 구체적인 정보를 알수 없다
-- TABLE JOIN을 통해서 구체적인 정보를 확인하자
-- EQ JOIN
SELECT * FROM tbl_book_rent BR, tbl_book BO, tbl_buyer BU
WHERE BR.br_isbn = BO.bk_isbn AND BR.br_bcode = BU.bu_code;

-- INNER JOIN
-- FK 관계로 설정된 TABLE간에 사용하는 JOIN
...
-- FK가 설정되지 않아도 데이터 조회는 모두 할 수 있다
-- 단, JOIN된 테이블에 데이터가 없으면 (NULL)로 표현된다
...
SELECT 대여일, 회원코드, 회원명, 반납일, bu.bu_tel FROM view_도서대여정보 BR
    LEFT JOIN tbl_buyer BU
...
-- 단순하게 보여달라
SELECT 대여일, 회원코드, 회원명, bu.bu_tel FROM view_도서대여정보 BR
    LEFT JOIN tbl_buyer BU
        ON BR."회원코드" = bu.bu_code
WHERE 대여일 < '2021-04-25' AND 반납일 IS NULL
...
VALUES(seq_book_rent.nextval,'2021-04-01','B0011','9791188850266');
