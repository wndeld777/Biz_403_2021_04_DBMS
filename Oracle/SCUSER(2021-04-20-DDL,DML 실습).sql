...
--          데이터의 길이가 모두 같은 경우
--          "고정길이 문자열"
--      VARCHAR : 저장되는 데이터의 최대 크기만 지정하고
--          최대크기보다 짧은 길이의 데이터를 저장할 수 있다
--          최대크기보다 짧은 데이터를 저장하면
--          자동으로 공간을 축소하여 데이터를 저장한다
--          "가변길이 문자열"
--      nVARCHAR : 한글과 같은 UNICODE 문자열을 저장할때
--          영문알파벳과 저장하는 방식이 달라서 문제를 일으킬수 있다
...
-- DML(Data Manipulation Lang. Data Management Lang, 데이터 조작어, 데이터 관리)
...
-- 데이터 읽기
SELECT sc_num, sc_kor, sc_eng, sc_math FROM tbl_score;
