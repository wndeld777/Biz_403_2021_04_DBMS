...
-- 보고자할때는 GROUP BY 를 사용하여 묶어줘야 한다
...
SELECT sc_num 학번, st_name, st_dept, sc_kor, sc_eng, sc_math
FROM tbl_score, tbl_student
WHERE sc_num = st_num;
...
-- 학생데이터에서 일부를 삭제한 후
-- JOIN을 실행하였더니 성적데이터가 79개 밖에 조회되지 않는다
-- 성적데이터는 모두 100개가 있지만
-- 학생데이터는 79개 만 남아 있기 때문에
-- JOIN한 결과가 학생데이터와 같은 수인 79개만 조회(인출)되고 있다
...
-- 학생데이터에는 1 ~ 79번 까지만 있고
-- 성적데이터에는 1 ~100번 까지 있다
-- 성적데이터의 80 ~ 100번 까지는
-- 실제 존재하는 학생인지 아닌지 증명할 방법이 없다
-- 성적데이터는 무결성이 깨진상태가 된다
-- 학생테이블과 성적테이블간의 연관(관계) 참조가 무너진 상태가 된다
--      == "참조 무결성 오류"가 발생했다 라고 표현한다
-- 참조 무결성에 문제가 생긴경우 JOIN을 했을때
--   인출되는 데이터의 신뢰성을 보증 할수 없다
SELECT * FROM tbl_score;
SELECT * FROM tbl_student;
...
-- 학생데이터에 없는 학생의 성적이 추가되어 있는지 여부를 알아보고 싶을때 
-- 참조무결성에 오류가 있는지 알고 싶을때
SELECT COUNT(*)
FROM tbl_score
...
-- ROUND(값,0)  : 소주점이하 반올림하고 정수형으로
-- ROUND(값, 자릿수) : 자릿수 이하값은 무조건 절사
...
-- 학과별로 묶고 각 학과의 총점을 계산
-- 평균도 계산
-- 학과 칼럼을 GROUP BY 로 묶지 않으면 오류가 발생
