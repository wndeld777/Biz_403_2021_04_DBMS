...
SELECT bk_comp AS 코드,COUNT(*),cp_name AS 출판사명
FROM tbl_books B
...
-- tbl_books 테이블에서
-- 도서가격이 2만원 이상인 도서들의 리스트
SELECT * FROM tbl_books
WHERE bk_price >= 20000;
-- 도서가격이 2만원 이상인 도서들의 전체 합계금액
SELECT SUM(bk_price)
FROM tbl_books
WHERE bk_price >=20000;

-- tbl_books, tbl_company, tbl_author 세개의 table JOIN하여
-- ISBN, 도서명, 출판사명, 출판사대표, 저자, 저자연락처 으로 출력되도록 SQL작성
...
-- tbl_books, tbl_company, tbl_author 세개의 table JOIN하여
-- ISBN, 도서명, 출판사명, 출판사대표, 저자, 저자연락처, 출판일 으로 출력되도록 SQL작성
-- 출판일이 2018년 데이터만
SELECT B.bk_isbn ISBN,
    B.bk_title 도서명,
    C.cp_name 출판사명,
...
-- PK를 WHERE절에 포함하여 UPDATE, DELETE 를 수행하자
-- 안전한 방법
UPDATE tbl_author SET au_tel = '010-9898-6428'
WHERE au_code = 'A0006';
