...
--  SYSDBA권한은 TableSpace를 만들고, User를 만드는 역할
-- SYS 사용자로 접속한 상태
...
-- 데이터를 추가할수 있도록 설정
-- scuser 계정으로 데이터를 추가하면 자동으로 schoolDB tablespace에 데이터가 모이게 된다.
...
--  아무것도 할수 없는 상태이다.
--  관리자(SYSDBA)는 새로 생성된 계정이 무언가 명령을 수행할수 있다
...
-- GRANT ** TO user : user 에게 ** 권한을 부여한다.
