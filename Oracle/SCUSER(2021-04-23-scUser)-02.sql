...
-- Oracle Server 와 SQL Developer 사이에서 중간에 임시로 데이터가 보관되는 상태가 된다
...
DELETE FROM tbl_score; -- tbl_score 의 모든데이터를 삭제
...
-- 또한 특별하게 필요한 경우가 아니면
-- WHERE절에 포함하는 조건은  PK 칼럼을 기준으로 하자
...
-- VIEW 만들때 설정한 AS 값들이 칼럼이름처럼 사용된다
SELECT * FROM view_score
WHERE 총점 < 200;
-- 총점이 200미만인 학생들 데이터를 조회하여
-- 총점 순으로 오름차순 정렬하여 보여라
SELECT * FROM view_score
WHERE 총점 < 200
ORDER BY 총점;
...
WHERE 총점 < 200
ORDER BY 영어 DESC, 수학, 국어; -- DESC 내림차순

-- 총점이 150보다 크고 200보다 작은 점수만 보여라
SELECT * FROM view_score
WHERE 총점 > 150 AND 총점 < 200;

-- 총점이 150 이상이고 200이하인 점수만 보여라
SELECT * FROM view_score
WHERE 총점 >= 150 AND 총점 <= 200;
...
-- 국어점수가 70점 이상인 데이터
SELECT * FROM view_score
WHERE 총점 BETWEEN 150 AND 200 AND 국어 >= 70; -- 조건검색, 조건조회
...
-- 학번은 단독으로 사용되었다
-- 이러한 데이터를 출력하려면
-- 함수로 감싸지 않은 칼럼들을 GROUP BY에 나열해 주어야한다
SELECT 학번, MAX(총점)
FROM view_score
GROUP BY 학번;
