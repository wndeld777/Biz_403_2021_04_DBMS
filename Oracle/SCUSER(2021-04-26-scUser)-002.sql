...
SELECT sc_num AS 학번,
    (SELECT st_name FROM tbl_student WHERE sc_num = st_num) AS 이름,
    (SELECT st_dept FROM tbl_student WHERE sc_num = st_num) AS 학과
...
-- 학생점수를 저장할 테이블을 정규화를 하기 위해 재 설계를 하였다
...
-- 이 테이블은 학번 + 과목명을 묶어서 PK로 선언을 해야 할수 밖에 없게 되었다
-- PK는 단독 칼럼으로 사용하는 것이 가장 좋은데
-- 단독 칼럼으로 만들수 없는 경우는 2개이상의 칼럼을 묶어서 
-- 만들어야 하는 경우도 있다
